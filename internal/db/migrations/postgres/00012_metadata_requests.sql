-- PostgreSQL migration 00012: metadata provider settings, request profiles, requests, and quotas.
...
CREATE TABLE request_profiles (
    ...
    accepts_movies BOOLEAN NOT NULL,
    accepts_series BOOLEAN NOT NULL,
    download_manager_kind TEXT NOT NULL CHECK (octet_length(download_manager_kind) BETWEEN 1 AND 500),
    download_manager_instance TEXT NOT NULL CHECK (octet_length(download_manager_instance) BETWEEN 1 AND 500),
    quality_profile TEXT NOT NULL CHECK (octet_length(quality_profile) BETWEEN 1 AND 500),
    root_folder TEXT NOT NULL CHECK (octet_length(root_folder) BETWEEN 1 AND 500),
    created_at TIMESTAMPTZ NOT NULL,
    updated_at TIMESTAMPTZ NOT NULL,
    CHECK (accepts_movies OR accepts_series)
);
...
CREATE TABLE request_profile_tags (
    ...
    tag TEXT NOT NULL CHECK (octet_length(tag) BETWEEN 1 AND 100),
...
CREATE TABLE requests (
    ...
    title TEXT NOT NULL CHECK (octet_length(title) BETWEEN 1 AND 500),
    release_year INTEGER NOT NULL CHECK (release_year BETWEEN 0 AND 9999),
    poster_path TEXT NOT NULL CHECK (octet_length(poster_path) <= 500),
    ...
    decision_reason TEXT NOT NULL DEFAULT '' CHECK (octet_length(decision_reason) <= 1000),
...
CREATE TABLE role_request_quotas (
    ...
    movie_period_seconds BIGINT NOT NULL CHECK (movie_period_seconds >= 0),
    season_limit INTEGER NOT NULL CHECK (season_limit >= 0),
    season_period_seconds BIGINT NOT NULL CHECK (season_period_seconds >= 0),
