-- SQLite migration 00015: notification channels and durable delivery outbox.
...
CREATE TABLE notification_channels (
    ...
    name_key TEXT NOT NULL COLLATE BINARY CHECK (length(CAST(name_key AS BLOB)) BETWEEN 1 AND 300),
    target TEXT NOT NULL CHECK (length(CAST(target AS BLOB)) <= 2048),
    settings_json TEXT NOT NULL CHECK (length(CAST(settings_json AS BLOB)) BETWEEN 2 AND 16384),
...
CREATE UNIQUE INDEX notification_channels_name_key_active_idx ON notification_channels (name_key COLLATE BINARY) WHERE deleted_at IS NULL;
CREATE INDEX notification_channels_name_key_id_idx ON notification_channels (name_key COLLATE BINARY, id) WHERE deleted_at IS NULL;
