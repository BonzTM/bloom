...
JOIN media_servers AS ms ON ms.id = amu.media_server_id
WHERE amu.account_id = sqlc.arg(account_id)
  AND (CAST(sqlc.arg(include_suppressed) AS INTEGER) = 1 OR amu.suppressed_at IS NULL)
...
-- name: SuppressAccountMediaUser :execrows
UPDATE account_media_users
SET suppressed_at = sqlc.arg(suppressed_at),
