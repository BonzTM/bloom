...
| `src/features/invites/`           | `/api/v1/invites` and public `/api/v1/invite/{code}` wire schemas, API slice, the account-scoped paged query with create and revoke, the public preview and accept flow, the invite form, table, and one-time link                                                         |
| `src/features/playback/`          | `/api/v1/playback/now` and `/history` wire schemas, API slice, the account-scoped live query and paged history, formatting of items, positions, and watch time, and the two tables                                                                                         |
| `src/features/media-servers/`     | `/api/v1/media-servers` wire schemas, API slice, the account-scoped paged query with register and remove mutations, the registration form, and the servers table                                                                                                           |
