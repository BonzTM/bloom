...
- Three feature modules: `system` consumes `GET /api/v1/version` and renders the server version badge; `auth` signs in and out against `/api/v1/auth`, holds the session with its roles and permissions, and gates navigation on them; `roles` pages through `GET /api/v1/roles` for the administration area.
...
| `src/features/auth/`              | sign-in and sign-out: wire schemas mirrored from OpenAPI, API slice, the session query with its roles and permissions and principal-scoped cache clearing, permissions, login form, session controls, permission gate |
